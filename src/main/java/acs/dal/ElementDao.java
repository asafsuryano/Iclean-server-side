...
public interface ElementDao extends PagingAndSortingRepository<ElementEntity,ElementId> {
	...
	//select .... from ElementEntity where element is within circle for managers
	@Query(value="select * from Element  where SQRT((lat - :lat )*(lat - :lat )+(lng - :lng )*(lng - :lng )) <= :distance and type like '%location%'",nativeQuery = true)
	public List<ElementEntity> findAllDirtyLocationsByLocationNear(@Param("lat") double lat,@Param("lng") double lng
			,@Param("distance") double distance,Pageable pageable);
	...
	//select .... from ElementEntity where element is within circle for players
	@Query(value="select * from Element  where SQRT((lat - :lat )*(lat - :lat )+(lng - :lng )*(lng - :lng )) <= :distance and active=true and type like '%location%' ",nativeQuery = true)
	public List<ElementEntity> findAllDirtyLocationsByLocationNearAndActive(@Param("lat") double lat,@Param("lng") double lng
			,@Param("distance") double distance,Pageable pageable);
}
